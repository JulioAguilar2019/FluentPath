...
export async function savePomodoroSessionAction(input: SavePomodoroSessionInput) {
  ...
  if (!Number.isInteger(input.focusMinutes) || input.focusMinutes <= 0) {
    throw new Error("Focus minutes must be greater than zero.");
  }
  ...
  if (!Number.isInteger(input.pomodoroCycles) || input.pomodoroCycles <= 0) {
    throw new Error("At least one completed pomodoro cycle is required.");
  }
  ...
  const { error } = await supabase.from("study_sessions").insert({
    ...
    pomodoro_cycles: input.pomodoroCycles,
    notes: input.notes?.trim() ? input.notes.trim() : null,
  });
